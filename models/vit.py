...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 ckpt_layer=0, t_or_s=None, kd_token=None):
        """
        Args:
    ...
    def forward(self, x, register_blk=-1, prompt=None, kd_prompt=None, q=None, train=False, task_id=None, t_p_list_=None, project_fc_layers=None, project_t_s=None, t_corr_list_=None, kd_tokens_=None, KD_method=None):
        ...
        prompt_loss_ = torch.zeros((1,), requires_grad=True).cuda()
        ...
        for i,blk in enumerate(self.blocks):
            ...
            if(self.t_or_s==0):
                ...
                if KD_method == 'ReviewKD':
                    t_features_list.append(x.clone().unsqueeze(1))
                elif KD_method == 'FitNets':
                    t_features_list.append(x)      
                else :
                    t_features_list.append(None)
                ...
                if((t_p_list_ is not None) and (t_p_list_[i] is not None)):    
                    ...
                    if KD_method == 'ReviewKD':
                        ...
                        s_features_list.append(x)
                    else :
                        s_features_list.append(None)
            else:
                x = blk(x, register_blk==i, prompt=p_list)
