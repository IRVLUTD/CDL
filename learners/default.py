...
class NormalNN(nn.Module):
    ...
    def learn_batch(self, train_loader, train_dataset, model_save_dir, args, val_loader=None):
        ...
        if need_train:
            ...
            s_acc = AverageMeter()
            soft_loss_curss = AverageMeter()
            soft_loss_press = AverageMeter()
            hint_losses = AverageMeter()

            # Train the Teacher part
            for epoch in range(self.config['schedule'][-1]):
                ...
                for i, (x, y, task)  in enumerate(train_loader):
                    ...
                    # model update
                    loss, output, cur_logits, p_list_, t_corr_list_, t_features_list= self.update_model(x, y, epoch) 
            ...
            for epoch in range(self.config['schedule'][-1]):
                ...
                if epoch > 0: self.s_scheduler.step()
                ...
                for i, (x, y, task)  in enumerate(train_loader):
                    ...
                    hint_losses.update(hint_loss_all, y.size(0))


                # eval update
                self.log(' * Student_Loss {loss.avg:.3f} | Soft_Loss {soft_loss.avg:.3f} | Hint_Loss {hint_loss.avg:.3f} | Train Acc {acc.avg:.3f}'.format(loss=s_losses, soft_loss=soft_losses, hint_loss=hint_losses, acc=s_acc))
    ...
    def validation(self, dataloader, model=None, task_metric='acc',  verbal = True, task_global=False, t_or_s=None, t_p_list_=None):
        ...
        for i, (input, target, task) in enumerate(dataloader):
            ...
            output, _ , _,_= model.forward(input)
